...
<div ng-app="app" class="index">
    {{ Form::open(array('url' => $basepath.'/disable', 'method' => 'POST')) }}
        <div ng-controller="WarehouseController" class="my-controller">
            <div class="page-actions">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="no-top pull-left no-pull-xs">All Warehouses</h1>
            ...
            <div class="row">
                <div class="col col-md-12">
                    <table class="table">
                        ...
                        <tbody>
                            <tr ng-class="{highlight: warehouse.new == 1}" dir-paginate-start="warehouse in warehouses | filter:search | orderBy: '-updated_at' | orderBy:orderByField:reverseSort | itemsPerPage: pageSize" current-page="currentPage">
                                @if (Auth::check() && Auth::user()->hasRole(['Superadmin', 'Admin']))
                                    <td ng-click="checkbox()">
                                        <input class="bulk-check" type="checkbox" name="ids[]" value="@include('_helpers.warehouse_id')">
                                    </td>
                                @endif
                                
                                <td>
                                    @if (Auth::check() && Auth::user()->hasRole(['Superadmin','Admin']))
                                        <a href="/{{$basepath}}/@include('_helpers.warehouse_id')"><span ng-bind="warehouse.name"></span></a>
                                    @else
                                        <a target="_blank" href="/{{$basepath}}/@include('_helpers.warehouse_id')"><span ng-bind="warehouse.name"></span></a>
                                    @endif
                                </td>
                                <td>
                                    <span ng-bind="warehouse.address.address_1"></span>
                                </td>
                                <td>
                                    <span ng-bind="warehouse.address.address_2"></span>
                                </td>
                                <td>
                                    <span ng-bind="warehouse.address.city"></span>
                                </td>
                                <td>
                                    <span ng-bind="warehouse.address.state"></span>
                                </td>
                                <td>
                                    <span ng-bind="warehouse.address.zip"></span>
                                </td>
                                @if (Auth::check() && Auth::user()->hasRole(['Superadmin', 'Admin']))
                                    
                                    <td class="hidable-sm boolean border">
                                        <span ng-if="warehouse.disabled"><i class="fa fa-check"></i></span>
                                    </td>
                                    
                                    <td>
                                        <span ng-bind="warehouse.updated_at"></span>
                                    </td>
                                @endif
                            </tr>
...
<script>
    ...
    var all_warehouses = "{{\Config::get('inventory::config.route.api.all-warehouses')}}";
    var path = "/"+api_path+"/"+all_warehouses;

    var app = angular.module('app', ['angularUtils.directives.dirPagination']);
    
    function WarehouseController($scope, $http, $filter) {
    
        $http.get(path).success(function(v) {
            $scope.warehouses = v.data;
        ...
        $scope.savePDF = function(){
            var selected = $filter('filter')($scope.warehouses,  $scope.search);
            sample = "warehouse=";
